...
public class FilteringExample {
	public static void main(String[] args) {
		...
		List<Member> list = Arrays.asList(	//Arrays.asList는 add를 안해줘도 알아서 넣어줌
				new Member("홍길동", Member.MALE, 30),	//MALE이 static이기 때문에 객체생성없이 바로접근
		...
		double ageAvg = list.stream()
				.filter(m -> m.getSex() == Member.MALE)	//남성 객체로만 구성된 스트림 생성	//성별이 int타입이라 ==로 비교
				//.mapToInt(m -> m.getAge()) 바로 아래거랑 같은 거임
