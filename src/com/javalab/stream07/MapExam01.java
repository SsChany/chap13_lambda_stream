...
public class MapExam01 {
	...
	public static void main(String[] args) {
		...
		List<String> highCaloricDish =
				...
				.map(m->m.getName())	//[중간연산]map() : 요리명만 추출된 별도의 stream생성
				//.map(menu::getName)	//요리명 추출[동일한 결과]
				.limit(3) 	//[중간연산] 선착순 세개만 선택
				.collect(Collectors.toList());	//[최종연산] collect나 forEach는 한번쓰고 나면 끝남	//stream으로 되있던게 list로 바뀐다
