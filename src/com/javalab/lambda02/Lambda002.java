...
interface MyFunctionalInterface{
	public int max(int a, int b); // 추상메소드
		...
		/*[람다식과 인터페이스형 함수]
		 * -람다식 : 함수 (메소드)를 간단한 '식(Expression)'으로 표현하는 방법
...
public class Lambda002 {
	public static void main(String[] args) {
		...
		//익명 구현 클래스 정의와 동시에 객체 생성
			...
			@Override	//부모 인터페이스 갖고 있는 max() 오버라이딩
		 ...
		 * [함수형 인터페이스]
		 * -생성된 익명 객체를 어딘가에 저장해서 사용해야함.
		 * -바로 그 때 필요한 것이 함수형 인터페이스임.
		 * -람다함수 즉, 익명 구현 객체의 매개변수와 반환형을 검증.
		 * (유추해서 알 수 있음)
		 */
		//익명 함수를 구현하는 익명 객체를 함수형 인터페이스 타입 변수에 저장함.
		//result 키워드를 사용하는 경우 or 사용 안하는 경우 [동일한 결과]
		//fi = (a, b) -> {return a> b ? a : b ;}; // [동일 결과]
		fi = (a, b) -> a > b ? a : b; // [동일 결과]	//익명 구현 객체  태어남.(논리적)
		...
		Object obj = new Object() {
		 ...
		 //obj.max(2, 3); // Object 클래스에는 max()메소드 없음.
	      
	      // 람다식으로 선언하여 fi에 저장한 익명 구현 객체의 max()호출

		max = fi.max(2,3);
		System.out.println("람다식을 선언한 익명 구현 객체 (fi)의 max() 호출 : " + max);
		System.out.println(fi)
		;
